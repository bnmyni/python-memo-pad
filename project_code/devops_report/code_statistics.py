...
def main():
    ...
    product = sys.argv[1].strip() if len(sys.argv) > 1 else None
    baseline = sys.argv[2].strip() if len(sys.argv) > 2 else None
    vss_path = sys.argv[3].strip() if len(sys.argv) > 3 else None
    component = sys.argv[4].strip() if len(sys.argv) > 4 else ""
    new_url, old_url, zt_version = get_svn_path(product, component, baseline, vss_path)
    print "old_url: %s" % old_url
    print "new_url: %s" % new_url
    print "zt_version: %s" % zt_version
